...
def start(message):
    if not functions.does_user_exist(str(message.from_user.id)):
        functions.user_registration('0', message)
...
def send_feedback(message):
    bot.send_message(message.chat.id, 'Следующим сообщением отправь отзыв, всё анонимно')
...
def waiting_for_pass(message):
    ...
    functions.set_value(message.from_user.id, 'Approved', 1)
    functions.set_value(message.from_user.id, 'STATUS', None)
    ...
    bot.send_message(message.chat.id, phrases.account_accepted, reply_markup=markup)
